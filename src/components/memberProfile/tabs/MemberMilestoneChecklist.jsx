...
import { getMilestonesByMember, deleteMilestone, assignMilestone as addMilestone } from '../../../api/milestoneRecords';
...
export default function MemberMilestoneChecklist({ memberId, onAddClick, refresh }) {
  ...
  const [adding, setAdding] = useState(false); // Track loading state for additions
  ...
  const handleAdd = async (templateId) => {
    setAdding(true);
    try {
      await addMilestone({ member_id: memberId, template_id: templateId });
      setSnackbar({ open: true, message: 'Milestone added successfully.', severity: 'success' });
      load(); // Reload milestones after addition
    } catch {
              ...
              return (
                <Tooltip key={t.id} title={tooltip}>
                  <Chip
                    ...
                    onClick={!completed ? () => handleAdd(t.id) : undefined} // Add milestone on click
